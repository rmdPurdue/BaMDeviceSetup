...
import static util.portEnumerations.UDP_RECEIVE_PORT;
...
public class DiscoveryQueryListener implements Runnable {
    ...
    public DiscoveryQueryListener() throws SocketException {
        // Set the UDP listning port
        int receivePort = UDP_RECEIVE_PORT.getValue();
    ...
    public void run() {
        ...
        while(running.get()) {
            try {
                ...
                // Set a test variable for finding a match to our newly found device in our exiting list of devices
                boolean matchTest = false;
                // Check if the MAC address of our newly-found device matches that of any devices we already know about
                for(RemoteDevice dev : remoteDevices) {
                    matchTest = dev.getMacAddress().equals(macAddress);
                }

                // If we don't have a device with the MAC address, add this new device to our list
                if(!matchTest) remoteDevices.add(new RemoteDevice(address, macAddress, deviceName));
